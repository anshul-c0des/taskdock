...
import { LayoutDashboard, Loader2, Mail, Lock, LogIn } from "lucide-react";
...
export default function LoginPage() {
  const { loginUser, loading } = useAuth();   // login user from useAuth hook
  ...
  const onSubmit = async (data: LoginForm) => {   // on submit handler
    try {
        ...
        <div className="bg-white p-6 sm:p-10 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div className="space-y-2">
              <Label
                htmlFor="email"
                className="text-xs font-medium uppercase tracking-wider text-slate-700"
              >
                Email Address
              </Label>
