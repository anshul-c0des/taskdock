...
export default function ProfilePage() {
  const { user } = useAuth();   // fetches current user
  const { data: tasks = [], isLoading } = useTasks();   // get tasks
  const router = useRouter();

  if (!user || isLoading) {   // page loading spinner
    return (
  ...
  const userTasks = tasks.filter(   // filter tasks which involves user
  ...
  const stats = [
    ...
    {
      label: "In Progress",
      count: userTasks.filter((t) => t.status === "IN_PROGRESS").length,
      icon: CircleDot,
  ...
  const recentTasks = [...userTasks]   // 5 recent tasks involving user
    .sort(
      (a, b) =>
    ...
    <div className="max-w-3xl mx-auto px-4 py-6 md:py-10 space-y-6 md:space-y-10">
          ...
          >
            <CardContent className="p-4 md:p-6 flex flex-col sm:flex-row items-center sm:justify-between gap-3 text-center sm:text-left">
              <div className="order-2 sm:order-1">
                ...
                <p
                  className={cn("text-2xl md:text-4xl font-black", stat.color)}
                >
                  {stat.count}
                </p>
              </div>
              <div
                className={cn(
                  "p-2.5 md:p-3 rounded-xl order-1 sm:order-2",
                  stat.bg
                )}
              >
                <stat.icon
                  className={cn("w-5 h-5 md:w-7 md:h-7", stat.color)}
                />
              </div>
            </CardContent>
          </Card>
      ...
      <div className="space-y-4">
        <div className="flex items-end justify-between px-1">
          ...
          <Link
            href="/dashboard"
            className="text-xs md:text-sm font-bold text-indigo-600 hover:underline underline-offset-4"
          >
            View All
          </Link>
        </div>
                ...
                <div className="flex items-center justify-between p-4 bg-white border border-slate-200 rounded-xl group-hover:border-indigo-300 group-hover:shadow-md transition-all active:scale-[0.98]">
                  <div className="flex flex-col gap-1.5 overflow-hidden">
                    ...
                    <div className="flex items-center gap-2">
                      <Badge
                        variant="secondary"
                        className="text-[9px] font-black px-1.5 py-0 bg-slate-100 text-slate-500 border-none uppercase"
                      >
