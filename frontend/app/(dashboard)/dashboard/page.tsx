...
const PriorityBadge = ({ priority }: { priority: string }) => {
  ...
  return (
    <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border uppercase tracking-wider", styles)}>
      {priority}
    </span>
  );
};
...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-tighter", styles)}>
      {status.replace("_", " ")}
    </span>
  );
};
      ...
      <div className="flex items-center gap-2 px-1 pb-2">
        ...
        {visibleTasks.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 bg-slate-50/50 rounded-3xl border-2 border-dashed border-slate-200">
            ...
            <p className="text-slate-400 font-medium tracking-tight">No tasks found matching your filters.</p>
          </div>
        ) : (
